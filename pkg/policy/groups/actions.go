...
func UpdateDerivativeCCNPIfNeeded(newCCNP *cilium_v2.CiliumNetworkPolicy, oldCCNP *cilium_v2.CiliumNetworkPolicy) bool {
	if !newCCNP.RequiresDerivative() && oldCCNP.RequiresDerivative() {
		...
		controllerManager.UpdateController(fmt.Sprintf("delete-derivatve-ccnp-%s", oldCCNP.ObjectMeta.Name),
			controller.ControllerParams{
				DoFunc: func(ctx context.Context) error {
...
func addDerivativePolicy(ctx context.Context, cnp *cilium_v2.CiliumNetworkPolicy, clusterScoped bool) error {
	...
	var (
		derivativePolicy   *cilium_v2.CiliumNetworkPolicy
	...
	for numAttempts := 0; numAttempts <= maxNumberOfAttempts; numAttempts++ {
		derivativePolicy, derivativeErr = createDerivativeCNP(ctx, cnp, clusterScoped)
		...
		statusErr := updateDerivativeStatus(cnp, derivativePolicy.ObjectMeta.Name, derivativeErr, clusterScoped)
		if statusErr != nil {
	...
	if clusterScoped {
		_, err = updateOrCreateCCNP(derivativePolicy)
	} else {
		_, err = updateOrCreateCNP(derivativePolicy)
	}

	if err != nil {
		statusErr := updateDerivativeStatus(cnp, derivativePolicy.ObjectMeta.Name, err, clusterScoped)
		if statusErr != nil {
			metrics.PolicyImportErrors.Inc()
			metrics.PolicyImportErrorsTotal.Inc()
	...
	err = updateDerivativeStatus(cnp, derivativePolicy.ObjectMeta.Name, nil, clusterScoped)
	if err != nil {
		scopedLog.WithError(err).Error("Cannot update status for derivative policy")
	}
	return err
