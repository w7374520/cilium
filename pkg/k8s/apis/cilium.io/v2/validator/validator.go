...
const (
	...
	warnEmptyToFromEndpointMessage = "It seems you have a CiliumClusterwideNetworkPolicy " +
		"with an empty to/from endpoint selector. The behavior of this selector has been " +
		...
		"detailed discussion on the topic, see GitHub issue #12844"
)
...
func (n *NPValidator) ValidateCCNP(ccnp *unstructured.Unstructured) error {
	...
	if resCCNP.Spec != nil {
		if containsEmptyToFromEndpoint(resCCNP.Spec) {
			logger.Warning(warnEmptyToFromEndpointMessage)
			return nil
		}
	}
	...
	if resCCNP.Specs != nil {
		for _, rule := range resCCNP.Specs {
			if containsEmptyToFromEndpoint(rule) {
				logger.Warning(warnEmptyToFromEndpointMessage)
				return nil
			}
		}
...
// containsEmptyToFromEndpoint returns true if a CCNP contains an empty endpoint selector
// in ingress/egress rules.
// For more information - https://github.com/cilium/cilium/issues/12844#issuecomment-672074170
func containsEmptyToFromEndpoint(rule *api.Rule) bool {
